...
export default function CambiarPassword({ navigation, route }) {
  ...
  const handleGuardar = async () => {
    ...
    if (passwordActual === passwordNueva) {
      Alert.alert('Error', 'La nueva contraseña debe ser diferente a la actual'); return;
    }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: t.bg }]}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={[styles.inputContainer, { backgroundColor: t.input, borderColor: t.inputBorder }]}>
          <Ionicons name="lock-open-outline" size={18} color={t.textMuted} style={{ marginRight: 10 }} />
          <TextInput
            style={[styles.input, { color: t.text }]}
          ...
          <View style={styles.strengthContainer}>
            {[1, 2, 3, 4].map(i => (
              <View key={i} style={[
                styles.strengthBar,
                {
                  backgroundColor: passwordNueva.length >= i * 3
                    ? passwordNueva.length >= 10 ? '#10B981'
                      : passwordNueva.length >= 6 ? '#F59E0B' : '#EF4444'
                    : t.bg3,
                },
              ]} />
            ))}
            <Text style={[styles.strengthText, { color: t.textMuted }]}>
              {passwordNueva.length < 6 ? 'Débil'
                : passwordNueva.length < 10 ? 'Regular' : 'Fuerte'}
            </Text>
          </View>
        )}
        ...
        <View style={[
          styles.inputContainer,
          {
            ...
            borderColor: passwordConfirm.length > 0
              ? passwordNueva === passwordConfirm ? '#10B981' : '#EF4444'
              : t.inputBorder,
          },
        ]}>
          <Ionicons name="checkmark-circle-outline" size={18}
            color={passwordConfirm.length > 0
              ? passwordNueva === passwordConfirm ? '#10B981' : '#EF4444'
              : t.textMuted}
...
const styles = StyleSheet.create({
  ...
  iconContainer: { alignItems: 'center', marginBottom: 32, marginTop: 8 },
  iconBox: { width: 80, height: 80, borderRadius: 24, alignItems: 'center', justifyContent: 'center', marginBottom: 12 },
  iconTitle: { fontSize: 18, fontWeight: '700', marginBottom: 6 },
  iconSub: { fontSize: 13, textAlign: 'center', lineHeight: 18 },
  label: { fontSize: 13, fontWeight: '600', marginBottom: 8, marginTop: 4 },
  inputContainer: { flexDirection: 'row', alignItems: 'center', borderWidth: 1.5, borderRadius: 12, paddingHorizontal: 14, height: 52, marginBottom: 8 },
  input: { flex: 1, fontSize: 15 },
  dividerLine: { height: 1, marginVertical: 16 },
  strengthContainer: { flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 12 },
  strengthBar: { flex: 1, height: 4, borderRadius: 2 },
  strengthText: { fontSize: 11, marginLeft: 4, width: 50 },
  errorText: { color: '#EF4444', fontSize: 12, marginBottom: 8, marginTop: -4 },
  saveBtn: {
